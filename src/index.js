...
export class Runtime {
   ...
   * @return {Message} a message that was generated by the runtime
   */
    ...
    while(!singleInstruction && !priv.IP.halted) {
      ...
      if (command == null) {
        ...
        if (command.var.type === 'get') {
          return new Messages.Variable.Load(command.var.name, command.var.value);
        } else if (command.var.type === 'set') {
          return new Messages.Variable.Save(command.var.name, command.var.value);
        }
        ...
        return new Messages.EndOfFile(command.stop);
      }
    }

    if (priv.IP.halted) return new Messages.Halt("Halt instruction encountered");
   ...
   * @returns {Object{string, value}} a dictionary of variable names and their values
   */
  get variables() {
